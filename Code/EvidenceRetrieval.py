...
class EvidenceRetrieval(object):
    ...
    def ReverseImageSearch(self, input_text, image_filepath, topN):
        ...
        regex = '^(?:https?:\\/\\/)?(?:[^@\\/\\n]+@)?(?:www\\.)?([^:\\/\\n]+)'
        clean_text_list, articlesummarylist, similaritylist = [], [], []
        articleurls = []
        ...
        print('\n******* Search Articles through Reverse Image Search *******')
        print(f'\n******* Found No. of articles = {len(articleurls)} *******')   

        for i, article_text in enumerate(clean_text_list[:topN]):

            print(f'\nPERFORMING ABSTRACTION - ARTICLE: {articleurls[i]} . . .')
...
def main(query):
    ...
    ER_logger = logging.getLogger()
    start = time.time()
    cwd = os.path.dirname(os.path.realpath(__file__))
    print(f'INITIALISE EVIDENCE RETRIEVAL PIPELINE . . .')
    ER_pipeline = EvidenceRetrieval(cwd, device, ER_logger)
