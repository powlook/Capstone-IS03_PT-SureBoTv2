...
def cat_pred():
    ...
    print('test_dataset info :', test_dataset.get_info())
    ...
    if config.LOSS.USE_OHEM:
        criterion = OhemCrossEntropy(ignore_label=config.TRAIN.IGNORE_LABEL,
                                     ...
                                     weight=test_dataset.class_weights).cpu()
    else:
    ...
    # Calculate Ave score and pred bool    
    heatmap = pred
    score = np.mean(pred)
    pred_bool = np.sum(pred > np.mean(pred)) / pred.size > 0.05 # If more than 5% of total area is elevated, flag as doctored
