...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(columns=['filename', 'cat_net', 'score','ground_truth'])
    
    for i, img_filepath in enumerate(files[14:]):
        ...
        pred_bool, score, heatmap = cat_inference(file, cat_net_root)

        print(i, file, pred_bool, score, ground_truth)
        
        df.loc[i] = file, pred_bool, score, ground_truth

    df.to_excel('validate_cat_net.xlsx')
